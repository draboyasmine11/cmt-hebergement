...
@Component({
    ...
    template: `
        @if (root() && isVisible() && item().label) {
            <div class="layout-menuitem-root-text">{{ item().label }}</div>
        }
        @if (item().label && (!hasRouterLink() || hasChildren()) && isVisible()) {
            <a [attr.href]="item().url" (click)="itemClick($event)" [ngClass]="item().class" [attr.target]="item().target" tabindex="0" pRipple class="flex items-center w-full">
                <i [ngClass]="item().icon" class="layout-menuitem-icon"></i>
                <span class="layout-menuitem-text">{{ item().label }}</span>
                @if (item().badge) {
                    <span class="ml-auto mr-2 px-1.5 py-0.5 rounded-full text-[9px] font-extrabold leading-none bg-[#00529B] text-white">
                        {{ item().badge }}
                    </span>
                }
            </a>
        ...
        @if (hasRouterLink() && !hasChildren() && isVisible()) {
            ...
            >
                ...
                <span class="layout-menuitem-text">{{ item().label }}</span>
                @if (item().badge) {
                    <span class="ml-auto mr-2 px-1.5 py-0.5 rounded-full text-[9px] font-extrabold leading-none"
                          [ngClass]="rla.isActive ? 'bg-white text-[#00529B]' : 'bg-[#00529B] text-white'">
                        {{ item().badge }}
                    </span>
                }
            </a>
